...
%}
function [xk, dk, ak, Bk, iWk, it] = CGM_parcial(x, f, df, amin, amax, p, c1, c2, iW, tol, itmax, icg, irc, nu, Q,parcial)
    it = 0;
    xk = [x]; Bk = [0]; ak = []; iWk = [];
    dfx = df(x); dk = [-dfx]; d = -dfx; 
    while norm(dfx) > tol & it < itmax
        [a, iWout] = BLS_parcial(x,f,df,d,amin,amax, p, c1, c2, iW, Q,parcial);
        x = x + a*d;
        dfx1 = dfx; % df k-1
        dfx = df(x); % df k
